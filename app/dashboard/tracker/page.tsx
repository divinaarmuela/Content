...
import { trackerChannel } from '@/app/inngest/channels'
import { fetchTrackerSubscriptionToken } from './actions'
...
const SOURCE_STYLE: Record<Asset['source'], string> = {
  published: 'bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-950/40 dark:text-blue-300 dark:border-blue-900',
  external: 'bg-violet-50 text-violet-700 border-violet-200 dark:bg-violet-950/40 dark:text-violet-300 dark:border-violet-900',
  manual: 'bg-zinc-100 text-zinc-600 border-zinc-200 dark:bg-zinc-800 dark:text-zinc-400 dark:border-zinc-700',
}
...
export default function TrackerPage() {
  ...
  // ── live: clicks and new assets push to this page; messages are hints,
  //    the authenticated API stays the single source of truth ──
  const { messages } = useRealtime({
    channel: trackerChannel,
    topics: ['activity'] as const,
    token: () => fetchTrackerSubscriptionToken(),
  ...
  useEffect(() => {
    ...
    if (!latest) return
    const d = latest.data as { kind: 'click' | 'asset'; label: string; ts: number }
    if (!d?.ts || seen.current.has(d.ts)) return
    seen.current.add(d.ts)
    if (d.kind === 'click') toast.info(`Click — ${d.label}`, { duration: 2500 })
    else toast.success(`New asset registered — ${d.label}`)
    void load(true)
  }, [messages.last, load])
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-3">
        <div>
          <h2 className="text-lg font-semibold tracking-tight">Content Register</h2>
        ...
        <div className="ml-auto flex items-center gap-2">
          <Badge variant="outline" className="gap-1.5 font-mono text-[11px] tabular-nums">
            ...
            <Table>
              ...
              <TableBody>
                {visible.map(a => (
                  <TableRow key={a.id}>
                    <TableCell>
                      <div className="max-w-72 truncate text-sm font-medium" title={a.title}>{a.title}</div>
                    ...
                    <TableCell className="hidden capitalize text-sm text-zinc-600 md:table-cell dark:text-zinc-400">
                      {a.platform ?? '—'}
                    </TableCell>
                    <TableCell>
                      <Badge variant="outline" className={`capitalize ${SOURCE_STYLE[a.source]}`}>{a.source}</Badge>
                    </TableCell>
                    <TableCell className="hidden whitespace-nowrap text-sm text-zinc-500 sm:table-cell dark:text-zinc-400">
                      {fmt(a.published_at ?? a.created_at)}
                    </TableCell>
                    <TableCell className="text-right font-mono text-sm tabular-nums">{a.clicks}</TableCell>
                    <TableCell>
    ...
    <Dialog open={open} onOpenChange={o => { if (!o) onClose() }}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid gap-1.5">
            <Label>Link destination <span className="text-xs text-zinc-400">(defaults to the client&rsquo;s website)</span></Label>
            <Input value={destUrl} onChange={e => setDestUrl(e.target.value)} placeholder="https://client-site.com/contact" />
          </div>
          <div className="grid gap-1.5">
            <Label>Live post URL <span className="text-xs text-zinc-400">(optional)</span></Label>
            <Input value={postUrl} onChange={e => setPostUrl(e.target.value)} placeholder="https://instagram.com/p/…" />
          </div>
          <div className="grid gap-1.5">
            <Label>Offer code <span className="text-xs text-zinc-400">(&ldquo;mention RENO10&rdquo; — offline attribution)</span></Label>
