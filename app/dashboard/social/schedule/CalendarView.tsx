...
/**
 * THE CALENDAR VIEW: one client's week, media on the left, hours on the right.
 *
 * One of the Schedule page's three views (`page.tsx` owns the pill rail and
 * `?view=`); this is the one that address has always opened on. The Week /
 * Month / List / Preview / Stories choice inside it is its own, and rides in
 * the address as `?look=` so it cannot collide with the page's `?view=`.
 *
...
export default function CalendarView() {
  ...
  const [view, setView] = usePersistedChoice<ScheduleViewName>(VIEW_KEY, VIEWS, 'Week', 'look')
  ...
  const rail = (
    <MediaRail
      ...
      waitingHref={scheduleViewHref('approvals', { client: clientId })}
