...
import {
  Bot, History, Loader2, Plus, SendHorizontal, Settings2, ShieldAlert,
...
export default function AssistantPage() {
  ...
  const [initial, setInitial] = useState<UIMsg[]>([])
  ...
  const openChat = async (id: string) => {
    if (id === chatId) { setHistoryOpen(false); return }
    const res = await fetch(`/api/assistant/chats/${id}`)
    if (!res.ok) { toast.error('Could not open that chat'); void refreshList(); return }
    const json = await res.json()
    setInitial(json.messages ?? [])
    setChatId(id)
    setHistoryOpen(false)
  }
  ...
  const list = (
    <ChatList chats={chats} activeId={chatId}
      onOpen={id => void openChat(id)} onAskDelete={setConfirmDelete} />
  )
  ...
  return (
    <div className="flex h-[calc(100vh-9rem)] gap-4">
      ...
      <div className="mx-auto flex w-full max-w-3xl min-w-0 flex-1">
        <Conversation key={chatId} chatId={chatId} initialMessages={initial}
          onResponseDone={() => void refreshList()} />
      </div>
