...
import {
  ...
  rowsFor, shootManagerChoices, stampLines, stampWords,
...
export function PlanParts({ batch, itemCount, booked, onPatch, onShots, team }: {
  ...
  const area = (field: keyof ShootSopBatch, placeholder: string, rows = 2) => (
    <Textarea key={String(batch[field] ?? '')} defaultValue={String(batch[field] ?? '')} rows={rowsFor(String(batch[field] ?? ''), rows, 14, 55)}
      ...
      <CardContent className="flex flex-col gap-3 p-4">
        ...
        <label className="flex flex-col gap-1 text-[12px] font-semibold">
          ...
          <Select value={batch.owner_id ?? 'none'} onValueChange={v => void onPatch('owner_id', v === 'none' ? null : v)}>
            <SelectTrigger className="h-auto min-h-11 whitespace-normal py-2 text-left [&>span]:line-clamp-none text-[15px] font-normal" aria-label="Account manager for this shoot"><SelectValue placeholder="Pick the account manager" /></SelectTrigger>
            <SelectContent>
