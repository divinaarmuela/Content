 ...
 * board, no "in production", no status buttons. The card is moved by the
 * account manager from the dashboard, never from here.
 */
export default async function EditingPortalPage({ params }: { params: Promise<{ token: string; id: string }> }) {
  ...
  return (
    <div className={`dbx ${archivo.variable} ${sometype.variable} min-h-screen bg-[#0a0a0a] text-white`}
      ...
      <main className="mx-auto flex w-full max-w-[1500px] flex-col gap-6 px-5 py-6 pb-24 sm:px-8 sm:pb-16">
        <div className="flex flex-wrap items-end gap-x-6 gap-y-2">
          ...
          <p className="pb-1 text-[13px] text-white/50">
