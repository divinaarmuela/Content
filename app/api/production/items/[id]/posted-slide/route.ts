...
/**
 * POST { url, live_url?, platform? } — ONE FILE OF A PIECE, POSTED BY HAND.
 *
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  return withRequestCache(async () => {
    try {
      ...
      const body = await req.json().catch(() => ({})) as { url?: unknown; live_url?: unknown; platform?: unknown }
      ...
      const progress = postedProgress(slides, new Set(), byHand)
      ...
      if (fullyPosted(progress)) {
        ...
        await upsertScheduleEntry(user, item, {
          platform, scheduled_at: new Date().toISOString(),
