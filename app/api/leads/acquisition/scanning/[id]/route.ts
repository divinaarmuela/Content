...
import {
  conversationRefusal, conversationView, MAILBOX_CANNOT_SEND, NOBODY_TO_REPLY_TO, REPLY_TEXT_MAX, replyDraft, replyHtml, replyRefusal, signatureHtml, withSignatureText,
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  return withRequestCache(async () => {
    try {
      ...
      return NextResponse.json({ ...conversationView(t.row as never, t.thread), can_send: mailboxCanSend(t.box), reply_to: replyDraft(t.thread, String(t.row.subject ?? ''))?.to ?? null, reply_refusal: replyRefusal(t.thread), signature: await mailboxSignature(t.box.email).catch(() => null) })
    } catch (e) {
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  return withRequestCache(async () => {
    try {
      ...
      // the mailbox's signature under the words, in both parts (the owner, 23 Sep 2026)
      const signature = await mailboxSignature(t.box.email).catch(() => null)
      const sent = await sendReply(t.box, { to: draft.to, subject: draft.subject, text: withSignatureText(text, signature), html: replyHtml(text) + signatureHtml(signature), threadId: draft.threadId ?? undefined, inReplyTo: draft.inReplyTo ?? undefined, references: draft.references ?? undefined })
