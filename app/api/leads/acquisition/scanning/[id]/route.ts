...
import { mailboxSignature } from '../../../../../lib/scan-settings'
import { fetchGmailSignature, fetchThread, mailboxCanSend, sendReply } from '../../../../../lib/gmail'
import { stripHtml } from '../../../../../lib/gmail-core'
import { logAcqEvent } from '../../../../../lib/acquisition'
import { prospectForSender } from '../../../../../lib/acquisition-core'
import {
  conversationRefusal, conversationView, MAILBOX_CANNOT_SEND, NOBODY_TO_REPLY_TO, REPLY_TEXT_MAX, pickSignature, replyDraft, replyHtml, replyRefusal, withSignatureText,
...
/** the signature a reply from this mailbox carries: what Gmail has for it, else what Settings typed */
async function signatureFor(box: Parameters<typeof fetchGmailSignature>[0]) {
  const [gmail, typed] = await Promise.all([fetchGmailSignature(box), mailboxSignature(box.email).catch(() => null)])
  return pickSignature(gmail, typed, stripHtml)
}
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  return withRequestCache(async () => {
    try {
      ...
      const sig = mailboxCanSend(t.box) ? await signatureFor(t.box) : null
      return NextResponse.json({ ...conversationView(t.row as never, t.thread), can_send: mailboxCanSend(t.box), reply_to: replyDraft(t.thread, String(t.row.subject ?? ''))?.to ?? null, reply_refusal: replyRefusal(t.thread), signature: sig?.text ?? null, signature_source: sig?.source ?? null })
    } catch (e) {
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  return withRequestCache(async () => {
    try {
      ...
      // the mailbox's signature under the words, in both parts: Gmail's own first, the typed one as the fallback (23 Sep 2026)
      const sig = await signatureFor(t.box)
      const sent = await sendReply(t.box, { to: draft.to, subject: draft.subject, text: withSignatureText(text, sig?.text ?? null), html: replyHtml(text) + (sig?.html ?? ''), threadId: draft.threadId ?? undefined, inReplyTo: draft.inReplyTo ?? undefined, references: draft.references ?? undefined })
