...
export function maySharePublicly(card: { status?: unknown; accepted_at?: unknown; accepted_round?: unknown; edit_round?: unknown; merged_into?: unknown } | null | undefined): boolean {
  if (!card) return false
  // a closed card — all its clips moved on to the handover card — shares nothing; that card does (18 Sep 2026)
  if (typeof card.merged_into === 'string' && card.merged_into) return false
