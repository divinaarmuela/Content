 ...
 * the client's editing portal plays the same clips). The browser's <video>
 * asks for pieces of the file with a Range header and expects them back as
 * 206 with a Content-Range; without that it cannot seek, and a comment's
 * marker cannot jump to its second. The Range goes straight through to
 * Drive and the answer straight back, with our credentials and none of them
 * in the page. Read only (trap 13): a GET of bytes, nothing more. Never
 * cached: a clip is re-exported, the page plays the new one.
 *
 ...
 *
 * Two routes call this — the team's /api/drive/stream (a signed-in team
 * member) and the client's /api/portal/stream (a portal token and a signed
 * clip) — and each does its own gate before asking for the bytes.
 */
