...
describe('the card’s page', () => {
  ...
  it('Select mode ticks files across every tab and opens them side by side, two across, each with its comments (16 Sep 2026)', async () => {
    ...
    expect(tiles).toContain('onChange={e => onSelect(t, pickKeyRound ?? shownRound, e.target.checked)}')
    // the folder to work from keys its picks apart from the versions, even for the same file (16 Sep 2026)
    expect(src('app/dashboard/board/FilesToWorkFrom.tsx')).toContain('noRounds pickRound={0} />')
