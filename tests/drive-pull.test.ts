...
describe('the job, the triggers and the pages (source pins)', () => {
  ...
  it('saving a folder anywhere starts its pull, after the response', () => {
    ...
    expect(src('app/api/production/items/[id]/route.ts')).toContain("startPullSoon({ kind: 'item', scopeId: id, folderUrl: patch.raw_assets_url, version: Number(data.current_version_number) || null, by: user.id })")
    expect(src('app/api/production/items/[id]/link/route.ts')).toContain("if (check.kind === 'drive') startPullSoon({ kind: 'item', scopeId: id, folderUrl: check.url, version: done.version, by: user.id })")
  ...
  it('the bar sits under the folder on the shoot page and the card; the editing portal plays our copies once they are here', () => {
    ...
    expect(box).toContain('<DrivePullBar kind={pullScope.kind} scopeId={pullScope.id} folderUrl={folder} mayStart={mayEdit && !!pullScope.id} onPulled={() => setPulled(true)} />')
    expect(box).toContain('showFolderFiles && !pulled && <DriveFolderFiles')
