...
describe('the slices, the keys and the words (pure)', () => {
  it('one row per folder; a slice is the next 128 MB of a file, and none once it is all here', () => {
    ...
    // a card's pull is its own row (16 Sep 2026)
    expect(pullId('1abc', 'card-1')).toBe('folder-1abc@card-1')
    expect(pullId('1abc', null)).toBe('folder-1abc')
...
describe('the job, the triggers and the pages (source pins)', () => {
  ...
  it('the server half reads Drive and writes only to R2 — never a Drive write', () => {
    ...
    expect(s).toContain("if (!same) files.push({ id: olds.length > 0 && round !== null ? `${f.id}-v${round}` : f.id, name: f.name, mime: f.mime, size: f.size, done: 0, url: null, status: 'waiting', upload_id: null, parts: [], version: round, modified: f.modified, md5: f.md5 })")
  ...
  it('the bar sits under the folder on the shoot page and the card; the editing portal plays our copies once they are here', () => {
    ...
    expect(box).toContain('copies={pulled} selected={selecting ? pickedKeys : undefined} onSelect={selecting ? pick : undefined} noRounds pickRound={0} />')
    ...
    expect(bar).toContain("useRow<DrivePull>('drive_pulls', folderId ? pullId(folderId, kind === 'item' ? scopeId : null) : null)")
    expect(src('app/lib/drive-pull.ts')).toContain("const scopeKey = opts.kind === 'item' ? opts.scopeId : null")
    expect(src('app/lib/editing-portal.ts')).toContain('pullId(folder.folderId, item.id)')
