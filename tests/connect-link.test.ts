...
describe('who the link is for (the owner, 15 Sep 2026: "for their business, or the client’s personal one")', () => {
  it('a link made for a person carries their id; the business carries nothing', async () => {
    const { connectLinkPath, parseFor } = await import('../app/lib/connect-link-core')
    expect(connectLinkPath('tok', ['instagram'], 'c-j')).toBe('/connect/tok?networks=instagram&for=c-j')
    expect(connectLinkPath('tok', ['instagram'], null)).toBe('/connect/tok?networks=instagram')
    expect(connectLinkPath('tok', ['instagram'], 'company')).toBe('/connect/tok?networks=instagram')
    expect(parseFor('c-j')).toBe('c-j')
    expect(parseFor('company')).toBeNull()
    expect(parseFor('../x')).toBeNull()
    expect(parseFor(undefined)).toBeNull()
  })
  it('the sync tags only the accounts NEW in that sync, and the public route carries `for` out and back (source pins)', async () => {
    const { readFileSync } = await import('node:fs')
    const pub = readFileSync('app/lib/publish.ts', 'utf8')
    expect(pub).toContain('...(isNew && opts.tagNewWith ? { contact_id: opts.tagNewWith } : {}),')
    const route = readFileSync('app/api/connect/[token]/route.ts', 'utf8')
    expect(route).toContain('connectLinkPath(token, allowed, forContact)')
    expect(route).toContain("syncSocialAccounts(client.id, client.social_profile_id, { tagNewWith: forContact })")
    const page = readFileSync('app/connect/[token]/ConnectNetworks.tsx', 'utf8')
    expect(page).toContain('body: JSON.stringify({ platform, networks: networksParam, for: forContact }),')
    const invite = readFileSync('app/api/social/connect/invite/route.ts', 'utf8')
    expect(invite).toContain('connectLinkPath(token, networks, forId)')
    const control = readFileSync('app/dashboard/social/ClientConnectLink.tsx', 'utf8')
    expect(control).toContain('This link is for')
    expect(control).toContain('connectLinkPath(token, picked, contactIdOf(linkFor))')
    const card = readFileSync('app/dashboard/clients/SocialChannels.tsx', 'utf8')
    expect(card).toContain('accountSections(clientName, accounts, contacts)')
  })
})
