...
describe('where rounds are opened, tagged and shown (source pins)', () => {
  ...
  it('the card page has a tab per finished edit handed in, beside the folder to work from — the newest open (16 Sep 2026)', async () => {
    ...
    expect(box).toContain('<DriveFolderFiles url={shownVersion.folderUrl || null} wide={wideFiles} reviewHref={reviewHref} approvedIds={approvedIds} copies={versionFiles} selected={selecting ? pickedKeys : undefined} onSelect={selecting ? pick : undefined} />')
  ...
  it('the editing portal and the card show the newest round with pills for the others', () => {
    ...
    expect(portal).toContain('const rounds = roundsOf(shown)') // …the version's own clips and the approved ones carried in (17 Sep 2026)
