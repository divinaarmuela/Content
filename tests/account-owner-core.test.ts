...
describe('whom the post is for, from the New post window to the Schedule window (the owner, 15 Sep 2026)', () => {
  ...
  it('the New post window offers the business, or a person with an account connected, and the card carries it', () => {
    ...
    // a post or an editing card alike carries whom it is for (15 Sep 2026)
    expect(d).toContain('...(contactIdOf(postFor) ? { for_contact_id: contactIdOf(postFor) } : {}),')
    const r = src('app/api/production/items/route.ts')
    expect(r).toContain("for_contact_id: typeof it.for_contact_id === 'string' && /^[A-Za-z0-9_-]{1,64}$/.test(it.for_contact_id)")
    expect(r).toContain('? it.for_contact_id')
