...
describe('bold words on the board (the owner, 15 Sep 2026)', () => {
  ...
  it('the toolbar has a Bold button and the card wears it (source pins)', async () => {
    ...
    expect(bar).toContain('aria-label="Bold" title="Bold — the highlighted words; with nothing highlighted, a switch: on, what you type is bold; off, plain again"')
...
describe('bold words inside a note (the owner, 15 Sep 2026: "just make it bold when they highlight it")', () => {
  ...
  it('the note draws the runs, Cmd/Ctrl+B works in the box, and the toolbar bolds a highlight first (source pins)', async () => {
    ...
    expect(card).toContain('<BoldableTextarea')
    const bar = readFileSync('app/dashboard/production/shoots/[id]/BriefCanvas.tsx', 'utf8')
    expect(bar).toContain('if (el instanceof HTMLTextAreaElement || el instanceof HTMLInputElement) {')
    ...
    expect(card).toContain('<BoldableInput')
...
describe('bold as a switch while typing (the owner, 15 Sep 2026: "when Bold is live at the top we type bold; unclick, it’s unbold")', () => {
  it('opens a run at the caret, steps out of it on the second press, and knows when it is on', async () => {
    const { boldModeToggle, inBoldRun } = await import('../app/lib/batch-brief-core')
    const on = boldModeToggle('Hello ', 6)
    expect(on).toEqual({ text: 'Hello ****', caret: 8, on: true })
    expect(inBoldRun(on.text, on.caret)).toBe(true)
    const typed = 'Hello **world**'
    const off = boldModeToggle(typed, 13)
    expect(off).toEqual({ text: typed, caret: 15, on: false })
    expect(inBoldRun(typed, 15)).toBe(false)
    expect(inBoldRun('plain', 3)).toBe(false)
  })
  it('the boxes are boldable, with the B on the box and Cmd/Ctrl+B; the toolbar presses the same (source pins)', async () => {
    const { readFileSync } = await import('node:fs')
    const card = readFileSync('app/dashboard/production/shoots/[id]/CanvasCard.tsx', 'utf8')
    expect((card.match(/<BoldableTextarea/g) ?? []).length).toBe(2)
    expect(card).toContain('<BoldableInput')
    expect(card).not.toContain('boldHotkey')
    const box = readFileSync('app/dashboard/production/shoots/[id]/BoldBox.tsx', 'utf8')
    expect(box).toContain('aria-label="Bold the highlighted words"')
    expect(box).toContain('onMouseDown={e => e.preventDefault()}')
    const bar = readFileSync('app/dashboard/production/shoots/[id]/BriefCanvas.tsx', 'utf8')
