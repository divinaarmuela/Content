...
describe('version-approval-core — approved clips carry forward (17 Sep 2026)', () => {
  ...
  it('the portal, the card and the share link read the same rule', () => {
    ...
    expect(portal).toContain('const carried = carriedInto(list, latest, approvedIdSet(clipApprovalsOf(item)), moved).map(c => ({ ...c, version: latest }))')
    ...
    expect(box).toContain('const carried = shownVersion ? carriedInto(allFinished, shownVersion.round, approvedSet, movedSet).map(f => ({ ...f, version: shownVersion.round })) : []')
    ...
    expect(share).toContain('return versionSet([...uploaded, ...copied], round, approvedIdSet(clipApprovalsOf(item)), movedIdSet(item))')
  })
})
